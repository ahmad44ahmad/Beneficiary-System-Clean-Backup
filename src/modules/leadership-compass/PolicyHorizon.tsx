...
const ACCENT_STYLES: Record<'rose' | 'amber' | 'teal', { bg: string; text: string; border: string }> = {
    rose:  { bg: 'bg-rose-50',   text: 'text-rose-800',  border: 'border-rose-400' },
    amber: { bg: 'bg-amber-50',  text: 'text-amber-800', border: 'border-amber-400' },
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-3">
                {signals.length > 0
                    ...
                    : <div className="bg-white dark:bg-slate-800 border border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 text-center text-[12px] text-slate-400">
        ...
        <article className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-3">
                ...
                <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3">
                    ...
                    <p className="text-[13px] font-semibold text-slate-800 dark:text-slate-100 leading-relaxed">
                ...
                {expanded && (
                    <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-700">
                        {/* الإشارات الضعيفة */}
                        <div>
                            <div className="flex items-center gap-1 text-[10.5px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">
                                <Info className="w-3 h-3" />
                                إشاراتٌ ضعيفة رُصِدت
                            </div>
                            <ul className="space-y-1 text-[12px] text-slate-700 dark:text-slate-200">
                        ...
                        <div className="bg-hrsd-teal/5 rounded-lg p-3">
                            ...
                            <ol className="space-y-1 text-[12px] text-slate-700 dark:text-slate-200 list-decimal list-inside">
                        ...
                        <div className="flex items-start gap-2 text-[11.5px]">
                            ...
                            <span className="text-slate-500">
                                <strong className="text-slate-700 dark:text-slate-200">المعنيّون:</strong>{' '}
                                {signal.stakeholdersInvolved.join(' · ')}
                            </span>
                        </div>

                        {/* المخاطرة لو أُهمِلت */}
                        <div className="flex items-start gap-2 text-[11.5px] text-rose-700 bg-rose-50 rounded-lg p-2.5">
                ...
                <div className="flex items-center gap-2 text-[11px] text-slate-500 pt-2 border-t border-slate-100 dark:border-slate-700">
