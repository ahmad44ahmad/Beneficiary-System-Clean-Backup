        ...
        >
            ...
            <div className={`${tone.bg} px-5 py-2.5 flex items-center justify-between flex-wrap gap-2`}>
                ...
                {decision.deadline && (
                    <span className={`text-[13px] font-semibold flex items-center gap-1.5 ${
                        ...
                        (daysLeft !== null && daysLeft <= 7) ? 'text-orange-700' : tone.text
                    }`}>
            ...
            <div className="p-5 md:p-6 space-y-5">
                ...
                <div className="bg-hrsd-teal/5 border-r-4 border-hrsd-teal rounded-lg p-4">
                    ...
                    <p className="text-[15px] font-semibold text-slate-900 dark:text-white leading-relaxed mb-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {decision.estimatedCostSar !== undefined && (
                        <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                            <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mb-0.5">التكلفة</div>
                    ...
                    {decision.estimatedSroi !== undefined && (
                        <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                            <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mb-0.5">SROI المتوقَّع</div>
                    ...
                    {decision.affectedBeneficiariesCount !== undefined && (
                        <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                            <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mb-0.5">المتأثّرون</div>
                            <div className="text-[15px] font-bold text-slate-900 dark:text-white">
                    ...
                    {decision.barrierTypesAddressed && decision.barrierTypesAddressed.length > 0 && (
                        <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
                            <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mb-0.5">العوائق المستهدَفة</div>
                ...
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg
                        ...
                        hover:bg-slate-50 dark:hover:bg-slate-700 text-[14px] font-medium transition-colors"
                >
                ...
                {expanded && (
                    <div className="space-y-5 pt-3 border-t border-slate-200 dark:border-slate-700">
                        ...
                        {decision.evidence.length > 0 && (
                            <section>
                                <h4 className="text-[14px] font-bold text-slate-900 dark:text-white mb-2.5 flex items-center gap-2">
                                ...
                                <div className="space-y-2">
                                    {decision.evidence.map((e, i) => (
                                        <div key={i} className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
                        ...
                        {decision.alternatives.length > 0 && (
                            <section>
                                <h4 className="text-[14px] font-bold text-slate-900 dark:text-white mb-2.5">
                                    البدائل المدروسة
                                </h4>
                                    ...
                                    {decision.alternatives.map((a, i) => (
                                        <div key={i} className="border border-slate-200 dark:border-slate-700 rounded-lg p-3">
                                            ...
                                            <div className="grid md:grid-cols-2 gap-3 text-[13px]">
                                                ...
                                                <div>
                                                    <div className="font-medium text-rose-600 mb-1">عيوب</div>
                                                    <ul className="space-y-0.5 text-slate-700 dark:text-slate-300">
                            ...
                            <div className="grid md:grid-cols-3 gap-3 text-[13px]">
                                {decision.consequences.ifApproved && (
                                    <div className="bg-hrsd-green/5 border border-hrsd-green/30 rounded-lg p-3">
                                        ...
                                        <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{decision.consequences.ifApproved.impact}</p>
                                        {decision.consequences.ifApproved.timeline && (
                                            <p className="text-[12px] text-slate-500 mt-1">🕒 {decision.consequences.ifApproved.timeline}</p>
                                        )}
                                    </div>
                                )}
                                {decision.consequences.ifDelayed && (
                                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                                        <div className="font-bold text-amber-700 mb-1.5">لو أجَّلت</div>
                                        <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{decision.consequences.ifDelayed.impact}</p>
                                        {decision.consequences.ifDelayed.risk && (
                                            <p className="text-[12px] text-amber-700 mt-1">⚠ {decision.consequences.ifDelayed.risk}</p>
                                        )}
                                    </div>
                                )}
                                {decision.consequences.ifRejected && (
                                    <div className="bg-rose-50 border border-rose-200 rounded-lg p-3">
                                        <div className="font-bold text-rose-700 mb-1.5">لو رفضت</div>
                                        <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{decision.consequences.ifRejected.impact}</p>
                                    </div>
                                )}
                            </div>
                        </section>
                        ...
                        {/* المعلومات الإداريّة */}
                        <section className="text-[12px] text-slate-500 dark:text-slate-400 flex flex-wrap gap-x-4 gap-y-1 pt-2 border-t border-slate-100 dark:border-slate-700">
            ...
            {/* أزرار الإجراء */}
            <div className="bg-slate-50 dark:bg-slate-900/50 px-5 py-3 border-t border-slate-200 dark:border-slate-700
                flex flex-wrap gap-2 justify-end">
                <button
                    type="button"
                    onClick={() => onAction?.('more_evidence', decision)}
                    className="px-3.5 py-2 rounded-lg text-[13px] font-medium text-slate-600 dark:text-slate-300
                        hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center gap-1.5 transition-colors"
                >
                ...
                <button
                    ...
                    className="px-3.5 py-2 rounded-lg text-[13px] font-medium text-amber-700
                        hover:bg-amber-50 flex items-center gap-1.5 transition-colors"
                >
                ...
                <button
                    ...
                    className="px-3.5 py-2 rounded-lg text-[13px] font-medium text-rose-600
                        hover:bg-rose-50 flex items-center gap-1.5 transition-colors"
                >
