            ...
            <div className="bg-white border-b p-4 flex justify-between items-center shadow-sm z-10">
                <div>
                    <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                        <Brain className="w-6 h-6 text-[#FCB614]" />
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3 text-sm">
                                ...
                                <div className="bg-[#2BB574]/10 p-3 rounded-lg border border-[#2BB574]/10">
                                    <span className="text-[#2BB574] block text-xs mb-1">الوضع الاجتماعي</span>
                            ...
                            {/* Smart Suggestions */}
                            <div>
                                <h4 className="text-xs font-bold text-[#FCB614] uppercase tracking-wider mb-2">مقترحات الذكاء الاصطناعي</h4>
                                    ...
                                    {suggestions.map((s, i) => (
                                        <div key={i} className="group relative bg-[#FCB614]/10 hover:bg-[#FCB614]/15 border border-[#FCB614]/20 p-3 rounded-lg transition-all cursor-pointer" onClick={() => addGoal(s)}>
                                            <div className="flex justify-between items-start">
                                                <span className="font-medium text-[#0F3144] text-sm">{s.title}</span>
                                                <Plus className="w-4 h-4 text-[#FCB614] opacity-0 group-hover:opacity-100 transition-opacity" />
                                            </div>
                                            <div className="mt-1 text-xs text-[#FCB614] flex items-center gap-1">
                ...
                <div className="col-span-6 p-6 overflow-y-auto">
                        ...
                        {plan.goals?.map((goal, _idx) => (
                            <Card key={goal.id} className="p-4 border-e-4 border-e-primary-500 shadow-sm hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <Button variant="ghost" size="sm" className="text-[#DC2626] hover:text-[#DC2626]" onClick={() => removeGoal(goal.id)}>حذف</Button>
                                </div>
                                ...
                                <div className="bg-gray-50 p-3 rounded-lg mt-2">
                                    <div className="flex justify-between text-xs mb-2">
                                        ...
                                        <span className={`font-bold ${goal.progress === 100 ? 'text-[#2BB574]' : 'text-primary-600'}`}>{goal.progress}%</span>
                                    </div>
                                    <input
                                        type="range"
                                        min="0"
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-200">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <Crosshair className="w-5 h-5 text-[#FCB614]" />
                        ...
                        {plan.approvals?.map(approval => (
                            <div key={approval.role} className={`p-4 rounded-lg border transition-colors ${approval.status === 'approved' ? 'bg-[#2BB574]/10 border-[#2BB574]/20' : 'bg-gray-50 border-gray-200'}`}>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    {approval.status === 'approved' && <CheckCircle className="w-4 h-4 text-[#2BB574]" />}
                                </div>

                                {approval.status === 'approved' ? (
                                    <div className="text-xs text-[#2BB574]">
                                        <div>Approved by: {approval.approvedBy}</div>
                                        <div className="text-[#2BB574] opacity-75">{new Date(approval.approvedAt!).toLocaleDateString()}</div>
                                    </div>
                                ) : (
                                    <Button
