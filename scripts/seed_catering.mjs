...
// Use the hardcoded credentials to ensure it works immediately
const SUPABASE_URL = "https://ruesovrbhcjphmfdcpsa.supabase.co";
const SUPABASE_SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJ1ZXNvdnJiaGNqcGhtZmRjcHNhIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NTI4MzQxOSwiZXhwIjoyMDgwODU5NDE5fQ.XTieHy2Q6WUCU5AHo70QENHpklD0udvCFDRdFk01k8Q";
...
async function seedCatering() {
    ...
    const { data: beneficiaries, error: benError } = await supabase
        .table('beneficiaries')
        .select('id, full_name, medical_diagnosis');
    ...
    const plans = beneficiaries.map(b => {
        let type = 'قياسي';
        let special = '';
        const diagnosis = (b.medical_diagnosis || '').toLowerCase();
